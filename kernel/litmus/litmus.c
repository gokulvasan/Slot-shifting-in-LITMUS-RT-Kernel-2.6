...
asmlinkage long sys_set_rt_task_param(pid_t pid, struct rt_task __user * param)
{
	...
	target->rt_param.task_params.synced = 0;

	if( tsk = tsk_rt(target)->slot_shift_task) {
		if(PERIODIC != tsk->type) {
			printk(KERN_INFO "changing task state to interruptable\n");
		}
	}
	retval = 0;
      out_unlock:
...
asmlinkage long sys_reservation_create(struct res_data __user *type,
					void __user *config)
{
...
asmlinkage long sys_set_task_table_data(struct td_descriptor __user *_desc, void* param)
{
	struct td_descriptor desc;
	struct ss_task_struct *tsk;
	...
	ss_tsk = kmalloc(desc.size, GFP_KERNEL);

	if(copy_from_user(ss_tsk, param, desc.size))
		return -EFAULT;

	tsk = ss_task_struct_init(desc.type, ss_tsk, desc.pid );	
	if(!tsk)
		return -EFAULT;

	if(desc.type == PERIODIC) {
		if(add_task(&ss_data, tsk))
			return -EFAULT;
	}
	else {
		printk("APERIODIC Arrives");
	}

	printk("INITIATED: desc->pid: %d, desc->size: %d\n",
			desc.pid, desc.size);

	return 0;
}

asmlinkage long set_aper_sched_timeout(long delta)
{
	TRACE("SS_TRACE: SETTING TIMER FOR: %lld\n", delta);
	schedule_timeout_interruptible(delta);
}

asmlinkage long sys_slot_shift_aper_cnt (int res_id, unsigned int count)
{
	printk("SS_TRACE: APERIDIOC COUNT ARRIVES:%d\n", count);
	// this data needs to be passed to slot shit framework;
	litmus->slot_shift_aper_count(res_id, count);

	return 0;
}

asmlinkage long sys_slot_shift_add_interval(struct res_data __user *res_type,
						void __user *inj)
{
	return litmus->slot_shift_add_interval(res_type, inj);
}
