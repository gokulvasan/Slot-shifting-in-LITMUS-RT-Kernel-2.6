...
static long do_wait_for_ts_release(void)
{
	...
	TRACE("SS_PARSER: added to wait list\n");
	...
	TRACE("SS_PARSER: added to wait for completion\n");
...
static long do_release_ts(lt_t start)
{
	...
	TRACE("SS_TRACE: %lld <<<<<< synchronous task system release >>>>>>\n",
		litmus_clock());
...
asmlinkage long sys_wait_for_ts_release(void)
{
	...
	TRACE("Waiting for release called\n");
//	if (is_realtime(t)) commented for SS table injection.
